...
namespace aalapChat
{
    ...
    public partial class MainChatBox : Form
    {
        ...
        private int totalFrds=0,activeFrdMemId,msgLimit=0,passMatched=0;
        ...
        private void ButtonClick(object sender, EventArgs e)
        {
            ...
            chatListBox.Items.Add("Talking to : " + btn.Text + " [ " + activeFrdMemId.ToString()+" ] ");
            StartChat.Visible = true;
            StartChat.Enabled = true;
            ShowOldMsg.Visible = true;
        ...
        private void updateButton_Click(object sender, EventArgs e)
        {
            ...
            if (oldPassBox.Text.Length == 0 || newPassBox.Text.Length == 0 || newPassCheckBox.Text.Length == 0)
            {
                MessageBox.Show("All Fill passowrd boxes");
            }
            else 
            {
                if (passMatched == 1)
                {
                    string qu = "SELECT `password` FROM `membertable` WHERE `mem_id` = " + dbSet.mem_id();
                    if (dbNew.getMyPass(qu) == oldPassBox.Text)
                    {
                        string query = "UPDATE `membertable` SET `password`= " + newPassBox.Text + " WHERE `mem_id` = " + dbSet.mem_id() + " ";
                        dbNew.addMemberToContact(query);
                        labelEdit.Text = "Successful!!";
                        labelEdit.ForeColor = Color.Green;
                    }
                    else
                    {
                        MessageBox.Show("Old Passward is wrong");
                    }
                }
                else
                {
                    MessageBox.Show("New passward did not match");
                }
            }
        }
        ...
        private void newPassCheckBox_TextChanged(object sender, EventArgs e)
        {
            if (newPassCheckBox.Text == newPassBox.Text)
            {
                passWarning.Text = "Matched!!";
                passWarning.ForeColor = Color.Green;
